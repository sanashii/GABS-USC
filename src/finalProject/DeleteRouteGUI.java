        ...
        deleteRoute.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                try {
                    ...
                    String routeId = routeIDField.getText(); // Get the inputted route ID
                    String sql = "DELETE FROM routes WHERE route_id = " + routeId; // SQL query to delete the row with the given route ID

                    int result = statement.executeUpdate(sql); // Execute the SQL query and get the result

                    if (result == 1) { // If a row was deleted, show success message and dispose the frame
                        JOptionPane.showMessageDialog(jFrame, "Successfully deleted route with ID " + routeId + "!");
                        jFrame.dispose();
                    } else { // If no row was deleted, show error message
